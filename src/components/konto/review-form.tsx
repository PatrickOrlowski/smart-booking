...
export function ReviewForm({ bookingId }: { bookingId: string }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-border bg-background/60 p-3.5">
      <div className="meta-label">Oceń wizytę</div>
      <div className="mt-1.5 flex items-center gap-0.5" role="radiogroup" aria-label="Ocena w gwiazdkach">
        {[1, 2, 3, 4, 5].map((value) => (
          <button
            key={value}
            type="button"
      ...
      <Textarea
        ...
        placeholder="Jak było? (opcjonalnie)"
        maxLength={1000}
      ...
      >
        {isPending ? "Zapisywanie…" : "Dodaj opinię"}
      </button>
    </div>
  );
}
